...
class GPT(nn.Module):
    ...
    def forward(self, states, actions, targets=None, rtgs=None, 
        ...
        # for b in range(rtgs[13].view(-1, ).shape[0]):
        #     print(rtgs[b].view(-1, ))
        #     x = input()
        ...
        mask = states.reshape(-1, 3, grid, grid)[:, 2].reshape(states.shape[0], states.shape[1], grid * grid)  # The 3rd one obss_mask is used here raw
