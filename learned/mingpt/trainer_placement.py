...
from mingpt.utils import sample, my_sample
...
class Trainer:
    ...
    def __init__(self, model, train_dataset, test_dataset, config, assuming_cfg_idx):
        self.model = model
    ...
    def train(self):
        ...
        def run_epoch(split, epoch_num=0):
            ...
            for it, (x, y, r, t, m_x, b, st, cir, l) in pbar:
                ...
                if accs.mean() > self.best_acc + 0.02 and accs.mean()>=0.2:
                    ...
                    # torch.save(raw_model.state_dict(), "save_models_quad/{}-{:.3f}.pkl".format(strftime, accs.mean()))
                    torch.save(raw_model.state_dict(), "save_models_icelake_quad/{}-{:.3f}.pkl".format(strftime, accs.mean()))
                    model.train()
                
                # save model
                # if statistics.mean(losses) < self.best_loss:
                #     self.best_loss = statistics.mean(losses)
                #     strftime = time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())
                #     model.eval()
                #     raw_model = self.model.module if hasattr(self.model, "module") else self.model
                #     torch.save(raw_model.state_dict(), "save_models_quad/{}-{:.3f}.pkl".format(strftime, self.best_loss))
                #     model.train()
        ...
        if self.config.is_eval_only:
            ...
            # In our case only one benchmark
            for i, benchmark in enumerate(benchmark_list):
                print(i, benchmark)
                if i > 0:  # to only run adaptec 1
                    break
                ...
                for level in level_range:
                    ...
                    if not self.config.test_all_macro:
                        print("is not all macros")
                        
                        # This is the call to evaluate 
                        eval_return[benchmark][str(level)], T_scores = self.get_returns(level, self.test_dataset, benchmark = benchmark)
                        # return = 0, and benchmark = what database
    ...
    # This is where you call your model while you are evaluating of come up with a policy
    def get_returns(self, ret, test_dataset, is_single = False, benchmark = "adaptec1", 
        ...
        for i in range(repeat_num):
            ...
            # state = (3, 84, 84) => missing (?[b], ?[t], 3, 84, 84)
            # reward_sum = int,
            # meta_state = (2)
            ...
            if machine == 0:
                state_obs = torch.tensor(np.full((1, grid, grid), False))
                state_obs_s = torch.tensor(np.full((nFeatures, grid, grid), 0, dtype=np.float64))
                state_obs_mask = torch.tensor(np.full((1, grid, grid), True))

                state = torch.cat((state_obs, state_obs_s, state_obs_mask), 0).view(-1, grid, grid)

            else:
                state_obs = torch.tensor(np.full((1, 8, 10), False))
                state_obs_s = torch.tensor(np.full((nFeatures, 8, 10), 0, dtype=np.float64))
                state_obs_mask = torch.tensor(np.full((1, 8, 10), True))

                state = torch.cat((state_obs, state_obs_s, state_obs_mask), 0).view(-1, 8, 10)
            ...
            # This is the very first action you are taking
            # ------------------------------------------------------------
            sampled_action, action_probs = my_sample(
                self.model, state.unsqueeze(0), 1, temperature=1.0, sample=True, actions=None, 
            ...
            while True:
                ...
                # zz = input()
                state, reward, done, meta_state = my_env_step_new2(
                    x, m_x, st, actions, 
                    state, meta_state, reward,
                ...
                sampled_action, action_probs = my_sample(
                    self.model, state.unsqueeze(0), 1, temperature=1.0, sample=True, 
