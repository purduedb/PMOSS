...
struct BTree {
  ...
  restart:
    ...
    while (node->type==PageType::BTreeInner) {
      ...
      // -------------------------------------------------------------------------------------
      /**
       * TODO: Benchmark move_pages
       */
      
      /* #include <numaif.h>
      // Function to bind memory to a specific NUMA node
      template <typename T>
      void bind_memory_to_node(T* ptr, size_t size, int numa_node) {
      ...
      size_t object_size = sizeof(BTreeInner<Key>);

      // Bind the memory to the destination NUMA node
      bind_memory_to_node(inner, object_size, destNUMA);
      */
